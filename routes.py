...
@router.post('/user/')
async def post_user( id: str, address: str, fullname: str, age: str, email: str , db: Session = Depends(get_db)):
...
async def post_studentsdetails( id: int , db: Session = Depends(get_db)):
    return await service.post_studentsdetails(db , id)
